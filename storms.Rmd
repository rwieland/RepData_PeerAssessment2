...
This project is the second project in the Reproducable Research course on Coursera. The goal of this project is to use tools such as R markdown to produce reproducable research. To do so, we will be analyzing a data set as described per the project [instructions][4] below:
...
We can see that flods are responsible for the most total damage and property damage, but that drought is causes the most total crop damage. However, per event, hurricanes and typhoons cause more damage to both property and crops.
